...
% abstract superclass for model
    ...
    methods
        ...
        function [fx,Jac,V]=solveAtPoint(obj,solvec,point,mode,trans,expStruct,thisVI)
            % transition
            [nextst,outstr]=obj.calcStateTransition(point,solvec,0,trans,thisVI);
            % equations
            [fx,Jac,V]=obj.calcEquations(point(1),nextst,solvec,outstr,mode,expStruct);
        end        
        ...
        % Add newly defined parameters to the Params structure
        % USE WITH CARE: additional parameters should be set to values
        % consistent with existing solution
        % E.g. if original solution is based on y = x and the new parameter
        % a modified the equation to y = a x, params.a must equal 1
        function obj=augmentParams(obj,params)
            missingFields = setdiff(fieldnames(params),fieldnames(obj.Params));
            for i=1:length(missingFields)
                fieldname=missingFields{1};
                obj.Params.(fieldname)=params.(fieldname);
            end
        end
        ...
        function [resmat,VF,TF,failedPointsIndex]=solvePointList(mobj,...
				tmp_grid,tmp_resmat,tmp_transmat,tmp_vi,...
                tmp_refidx,Vtrans,print,logfname)
...
%            for i=1:gr_npt
                ...
                transpts = [(1:mobj.Exogenv.exnpt)',reshape(trans,mobj.Exogenv.exnpt,mobj.NSTEN)];
                expStruct = computeExpectations(mobj, point(1), transpts, vtrans);      
                thisVI=tmp_vi(i);
                
                % prepare function handle
                fh_solveAtPoint=@(sol)solveAtPoint(mobj,sol,point,0,trans,expStruct,thisVI);
                
                % call to fsolve
                try
                    [x,fx,exfl]=fsolve(fh_solveAtPoint,gvec,options);
                catch E
					if strcmp(E.identifier,'optimlib:trustnleqn:UserJacUndefAtX0')
						tmpOptions=options;
						tmpOptions.Jacobian='off';
						[x,fx,exfl]=fsolve(fh_solveAtPoint,gvec,tmpOptions);
					else
						if print>0
                ...
                if exfl<1 || sum(isnan(fx)) > 0
                    ...
                    try
                        [xret,fx,exflret,count]=mobj.tryOtherGuesses(fh_solveAtPoint,gvec,options);
                    
                    catch E
						if strcmp(E.identifier,'optimlib:trustnleqn:UserJacUndefAtX0')
							tmpOptions=options;
							tmpOptions.Jacobian='off';
							[xret,fx,exflret,count]=mobj.tryOtherGuesses(fh_solveAtPoint,gvec,tmpOptions);
						else
							if print>0
                ...
                [~,~,V]=solveAtPoint(mobj,xr,point,1,trans,expStruct,thisVI);
        ...
        % solve model at list of points
        function [resmat_failed,VF_failed,TF_failed,n_succ]=solvePointListFailed(mobj,...
				grid,index_failed,resmat,vi,tmp_transmat,...
				tmp_refidx,Vtrans, npasses,print,logfname)
            
            gr_npt=size(grid,1);
            resmat_failed=zeros(length(index_failed),size(resmat,2));
            VF_failed=zeros(length(index_failed),mobj.Vfct.Nof);
            TF_failed=zeros(length(index_failed),mobj.Tfct.Nof);
...
%                 'MaxIter',100,'MaxFunEvals',20000,'Algorithm','trust-region-dogleg');            
            ...
            for p=1:npasses
                ...
                this_VF_failed=zeros(length(index_failed),mobj.Vfct.Nof);
                this_TF_failed=zeros(length(index_failed),mobj.Tfct.Nof);
